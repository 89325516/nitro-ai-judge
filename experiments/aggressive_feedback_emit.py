...
def materialize(root: Path, spec: dict, args) -> dict:
    ...
    cmd = ["python3", str(source), "--train", str(args.train), "--test", str(args.test), "--base-output", str(args.base_output), "--high-risk-base-output", str(args.high_risk_base_output), "--anchor-output", str(args.anchor_output), "--best-output", str(args.best_output), "--output", str(output)]
    subprocess.run(cmd, cwd=root, check=True)
    row = {key: value for key, value in spec.items() if key != "config"}
    row.update({"source_file": str(source.relative_to(root)), "output_file": str(output.relative_to(root)), "pinned_commit": PINNED_COMMIT, "source_urls": [f"{RAW_BASE}/words_dict_romanian_{int(subject):03d}.csv" for subject in SUBJECTS] + [f"{RAW_BASE}/words_dict_romanian_merged.csv"], "reproduction_command": " ".join(cmd).replace(str(root) + "/", "")})
    return row
...
def compare_outputs(root: Path, candidate: dict, baseline: Path) -> dict[str, float]:
    current = np.asarray([float(row["answer"]) for row in read_csv(root / candidate["output_file"])], dtype=float)
    base = np.asarray([float(row["answer"]) for row in read_csv(baseline)], dtype=float)
    corr = float(np.corrcoef(current, base)[0, 1]) if np.std(current) and np.std(base) else 0.0
    return {"corr_vs_099": corr, "mae_vs_099": float(np.mean(np.abs(current - base))), "p95_abs_vs_099": float(np.quantile(np.abs(current - base), 0.95))}
