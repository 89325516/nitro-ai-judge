...
try:
    from experiments.three_tier_external_data import OUTPUT_LIMIT, PINNED_COMMIT, RAW_BASE, SOURCE_LIMIT, describe, read_csv
except ModuleNotFoundError:
    from three_tier_external_data import OUTPUT_LIMIT, PINNED_COMMIT, RAW_BASE, SOURCE_LIMIT, describe, read_csv
...
from pathlib import Path
import numpy as np
CONFIG={config!r}; RAW_BASE={RAW_BASE!r}; WORD_RE=re.compile(r"^(.*)_([0-9]+)_page_([0-9]+)_([0-9]+)$"); FAMS=("arg","enc","ins","lit","popsci")
def rc(p):
...
def materialize(root: Path, spec: dict, args) -> dict:
    ...
    cmd = ["python3", str(source), "--train", str(args.train), "--test", str(args.test), "--best-output", str(args.best_output), "--output", str(output)]
...
def validate_candidate(root: Path, candidate: dict, test_ids: list[str], best_path: Path) -> dict:
    ...
    row = {**candidate, "source_bytes": (root / candidate["source_file"]).stat().st_size, "output_bytes": (root / candidate["output_file"]).stat().st_size, "row_count": len(output), "columns": list(output[0].keys()) if output else [], "ids_match_test_order": [row["datapointID"] for row in output] == test_ids, "answers_finite_non_negative": bool(np.all(np.isfinite(values)) and np.all(values >= 0.0)), "mae_vs_159": float(np.mean(np.abs(values - best))), "mean_delta_vs_159": float(np.mean(values) - np.mean(best)), "std_delta_vs_159": float(np.std(values) - np.std(best)), **{f"answer_{key}": value for key, value in describe(values).items()}}
