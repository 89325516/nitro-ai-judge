...
def combined_features(rows: list[dict[str, str]], encoder, width: int) -> list[dict[str, float]]:
    ...
    vectors = compute_row_vectors(rows, encoder, width)
    extra = vector_feature_dicts(vectors)
...
def run_smoke(args: argparse.Namespace) -> dict:
    ...
    vectors = compute_row_vectors(rows, encoder, args.feature_width)
    report = {
        "mode": "transformer_smoke",
        "score_type": "local_estimate",
        "model": args.model if args.encoder == "hf" else "deterministic",
        "row_count": len(rows),
        "feature_width": int(vectors.shape[1]) if vectors.ndim == 2 else 0,
...
def run_predict(args: argparse.Namespace) -> dict:
    ...
    encoder = make_encoder(args)
    train_features = combined_features(train_rows, encoder, args.feature_width)
    test_features = combined_features(test_rows, encoder, args.feature_width)
    ...
    return {"mode": "transformer_predict", "score_type": "local_estimate", "row_count": len(test_rows), "promoted": False}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run experiment-only Transformer feature checks.")
    subparsers = parser.add_subparsers(dest="mode", required=True)
    smoke = subparsers.add_parser("smoke")
    smoke.add_argument("--train", type=Path, required=True)
    smoke.add_argument("--limit-rows", type=int, default=12)
    smoke.add_argument("--report", type=Path, default=Path("reports/transformer_smoke.json"))
    add_common(smoke)
