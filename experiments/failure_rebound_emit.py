...
try:
    from experiments.three_tier_external_data import OUTPUT_LIMIT, PINNED_COMMIT, RAW_BASE, SOURCE_LIMIT, SUBJECTS, describe, read_csv
except ModuleNotFoundError:
    from three_tier_external_data import OUTPUT_LIMIT, PINNED_COMMIT, RAW_BASE, SOURCE_LIMIT, SUBJECTS, describe, read_csv
...
from pathlib import Path
import numpy as np
PINNED_COMMIT={PINNED_COMMIT!r}; RAW_BASE={RAW_BASE!r}; SUBJECTS={SUBJECTS!r}; CONFIG={config!r}
...
def materialize(root: Path, spec: dict, args) -> dict:
    ...
    cmd = ["python3", str(source), "--train", str(args.train), "--test", str(args.test), "--best-output", str(args.best_output), "--failed-output", str(args.failed_output), "--anchor-output", str(args.anchor_output), "--high-risk-base-output", str(args.high_risk_base_output), "--output", str(output)]
    subprocess.run(cmd, cwd=root, check=True)
    row = {key: value for key, value in spec.items() if key != "config"}
    row.update({"source_file": str(source.relative_to(root)), "output_file": str(output.relative_to(root)), "pinned_commit": PINNED_COMMIT, "reproduction_command": " ".join(cmd).replace(str(root) + "/", "")})
    return row


def validate_candidate(root: Path, candidate: dict, test_ids: list[str], best_path: Path, failed_path: Path) -> dict:
    ...
    failed = np.asarray([float(row["answer"]) for row in read_csv(failed_path)], dtype=float)
    row = {**candidate, "source_bytes": (root / candidate["source_file"]).stat().st_size, "output_bytes": (root / candidate["output_file"]).stat().st_size, "row_count": len(output), "columns": list(output[0].keys()) if output else [], "ids_match_test_order": [row["datapointID"] for row in output] == test_ids, "answers_finite_non_negative": bool(np.all(np.isfinite(values)) and np.all(values >= 0.0)), "mae_vs_099": float(np.mean(np.abs(values - best))), "mae_vs_129": float(np.mean(np.abs(values - failed))), "corr_vs_099": float(np.corrcoef(values, best)[0, 1]), **{f"answer_{key}": value for key, value in describe(values).items()}}
